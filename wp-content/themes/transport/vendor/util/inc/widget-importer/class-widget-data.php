...
class Theemon_Widget_Data {
    ...
    public static function init() {
        ...
        //@THEEMON add_action( 'admin_menu', array( __CLASS__, 'add_admin_menus' ) );
        //@THEEMON add_action( 'load-tools_page_widget-settings-export', array( __CLASS__, 'export_widget_settings' ) );
        //@THEEMON add_action( 'wp_ajax_import_widget_data', array( __CLASS__, 'ajax_import_widget_data' ) );
    }
    ...
    public static function add_admin_menus() {
        //import
        self::$import_page = call_user_func("add_management"."_page", 'Widget Settings Import', 'Widget Settings Import', 'manage_options', 'widget-settings-import', array(__CLASS__, 'import_settings_page'));
        // export
        self::$export_page = call_user_func("add_management"."_page",'Widget Settings Export', 'Widget Settings Export', 'manage_options', 'widget-settings-export', array(__CLASS__, 'export_settings_page'));
     ...
     */
    public static function ajax_import_widget_data($import_file = "") {
        $response = array(
        ...
        /* @THEEMON $import_file = isset( $_POST['import_file'] ) ? $_POST['import_file'] : false;
        ...
        //$import_file = get_template_directory() . '/vendor/data/charity-widgets.json'; //THEEMON

        $clear_current = true; //isset( $_POST['clear_current'] ); //@THEEMON

        if ($clear_current)
            self::clear_widgets();
        //@THEEMON : Checkwidgets

        $json_data = wptm_get_contents($import_file);
        $json_data = json_decode($json_data, true);
        $wptm_sidebar_data = $sidebar_data = $json_data[0];
        $widgets = self::select_all_widgets($sidebar_data); //@THEEMON
        ...
        if (!empty($sidebar_data[0])):
            update_option('sidebars_widgets', $wptm_sidebar_data);
        endif;

        $response['id'] = ( self::parse_import_data($sidebar_data) ) ? true : new WP_Error('widget_import_submit', 'Unknown Error');

        //@THEEMON $response = new WP_Ajax_Response( $response );
        //@THEEMON $response->send();
    }
    
    /**
     * @THEEMON new function
     * @param type $sidebar_data
     * @return array
     */
    ...
    public static function select_all_widgets($sidebar_data) {
        ...
        if (isset($sidebar_data)) :
            ...
            foreach (self::order_sidebar_widgets($sidebar_data) as $sidebar_name => $widget_list) :
                if (count($widget_list) == 0) {
                    continue;
                }
                $sidebar_info = self::get_sidebar_info($sidebar_name);
    ...
    public static function get_widget_settings_json() {
        ...
        $func_get_contents="file_get"."_contents";
        $file_contents = $func_get_contents($widget_settings['file']);
